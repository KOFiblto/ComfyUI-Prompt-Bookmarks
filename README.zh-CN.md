...
- 节点 ID 改变时，对外暴露字段可通过更稳定的绑定信息恢复，同时避免模糊猜测
- 一条收藏可同时保存多个字段，例如：
  - 主提示词
  - 运镜 / 动作提示词
  - 负面提示词
- 按 **工作流 + 分组** 整理提示词
- 支持手动创建提示词与卡片内即时编辑 (Edit)
- 支持多封面媒体轮播 (Carousel)，可自由 `[＋]` 添加与 `[✕]` 删除
- 内置全屏灯箱 (Lightbox) 预览，支持 `[❮]` / `[❯]` 翻页与键盘方向键
- 弹窗支持右下角自由拖拽缩放并居中，提示词文本框自动拉伸填满高度
- 零第三方依赖的本地 AES-256-GCM 主密码加密与侧边栏快速加锁 / 解锁
- 支持原生 SQLite 数据库 (`.db`) 一键导出与安全恢复
- 提供 `prompt-bookmarks-create` 窗口事件供 Asset 浏览器或菜单直接联动
- 可选“显示所有控件”开关，支持同时收藏 LoRA、步数、宽高比等参数组件
- 空分组可安全删除
- 搜索提示词
- 支持按最近使用、最近收藏、名称、使用次数排序
- 匹配工作流内一键应用 / 恢复
- 所有工作流均可复制提示词
- HTTP / 局域网环境下提供剪贴板降级方案
- 支持导出 / 导入可迁移的 JSON 备份
- SQLite 持久化，数据库存放在 ComfyUI user 目录，并支持轻量 schema migration
- 生成完成后可尝试自动关联图片 / 视频预览
- 视频预览按需延迟加载，并可选择静音自动播放
- 只保存媒体引用，**不会复制生成文件**
...
插件内部会记录节点 / widget 的绑定信息用于重新定位字段，但这些只是内部实现细节，用户不需要自己管理。
...
对已经暴露到外层的字段，0.2.0 会额外保存一个轻量绑定 key，并且只在匹配明确时恢复；不会为了“自动修复”去猜测隐藏的内部图结构。
...
常见图片格式以及 MP4、WebM、MOV、MKV、M4V 视频输出均可在 ComfyUI 返回标准文件元数据时被识别。视频资源会在接近侧边栏可视区域时再加载，也可以在设置中开启静音自动播放。

## 媒体轮播与内置查看器 (Lightbox)

- **多封面轮播 (Carousel)** — 每个提示词收藏可添加多张自定义图片或视频封面，支持 `[＋]` 上传与 `[✕]` 删除。
- **内置灯箱 (Lightbox)** — 点击侧边栏预览缩略图即可在 ComfyUI 内部全屏高清查看，无需在浏览器新标签页打开。

## 可调整大小的对话框

- **自由缩放与居中** — 弹窗支持右下角拖拽缩放并始终居中显示，尺寸自动保存在本地。
- **自适应文本框** — 提示词文本框自动拉伸填满窗口可用高度，窗口越大编辑空间越充裕。

## 数据库备份与导出

- **JSON 备份** — 便携式合并导出的 JSON 格式，包含所有工作流、提示词绑定及媒体引用。
- **原生数据库 (.db) 备份** — 支持在设置中一键直接导出和导入整个 SQLite 数据库文件。

## 密码保护与本地加密

- **AES-256-GCM 加密** — 支持使用主密码对提示词内容与私密备注进行本地加密（纯标准库，无需额外依赖）。
- **会话加锁与解锁** — 可随时锁定提示词库；解锁会话保存在内存中，明文密码绝不写入磁盘。

## JSON 备份

侧边栏设置中提供 **导出 JSON** 和 **导入 JSON**。

备份内容包括工作流、提示词字段绑定、分组、收藏内容、使用信息和媒体引用；**不会把生成的图片或视频文件复制进备份**。

导入采用合并方式：相同收藏 ID 会更新，不相关的现有收藏继续保留。因此它适合做手动备份，也适合在不同 ComfyUI 环境之间迁移个人提示词库，而不需要直接复制 SQLite 数据库。
...
因此更新、重新拉取或重装插件仓库不会删除你的提示词收藏。以后数据库结构升级会由插件自动执行 migration。
